...
class DeviceCanOpenOptions : public CanDeviceOptions {
 public:
  ...
  DeviceCanOpenOptions(const uint32_t nodeId, const std::string name, const unsigned int maxSdoTimeoutCounter = 1,
                       const unsigned int maxSdoSentCounter = 10, const uint16_t producerHeartBeatTime = 0,
  ...
  // maxSdoTimeoutCounter = timeout [s] * looprate [Hz] (looprate = rate of checkSanity(..) calls. In asynchronous mode this is 10Hz by
