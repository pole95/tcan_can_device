...
#include "tcan_can_device/DeviceCanOpenOptions.hpp"
...
class DeviceCanOpen : public CanDevice {
 public:
  ...
  enum class NMTStates : uint8_t { stopped = 1, preOperational = 2, operational = 3 };
   ...
   * @param sdoMsg	the SDO response message (Note that sdoMsg is not a complete instance of an SdoMsg, only the members defined in
   * CanMsg are initialized)
   */
